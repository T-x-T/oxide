...
impl super::WorldLoader for Loader {
	fn load_chunk(&self, x: i32, z: i32, block_states: &HashMap<String, Block>) -> Chunk {
		...
		let actual_chunk_length = i32::from_be_bytes([
			actual_chunk_length_bytes[0],
			actual_chunk_length_bytes[1],
			actual_chunk_length_bytes[2],
			actual_chunk_length_bytes[3],
		]);
		...
		for x in chunk_nbt.get_child("sections").unwrap().as_list() {
			...
			let block_palette_properties: Vec<Vec<(String, String)>> = block_palette
				.iter()
				.map(|x| {
					x.get_child("Properties")
						.unwrap_or(&crate::NbtTag::TagCompound(String::new(), vec![]))
						.get_compound_children()
						.iter()
						.map(|x| (x.get_description().to_string(), x.as_string().to_string()))
						.collect()
				})
				.collect();
			let block_palette_block_name: Vec<&str> = block_palette.iter().map(|x| x.get_child("Name").unwrap().as_string()).collect();
			let block_palette_state_id: Vec<u16> = block_palette
				.iter()
				.enumerate()
				.map(|(i, _)| data::blocks::get_block_state_id_from_raw(block_states, block_palette_block_name[i], &block_palette_properties[i]))
				.collect();
...
fn save_entity_region_to_disk(region: (i32, i32), chunks: &[&Chunk], dimension: &Dimension, path: PathBuf) {
	...
	if fs::exists(&region_file_path).unwrap() {
		...
		for i in 0..1024 {
			locations_table[i] = (
				u32::from_be_bytes([0, read_file_bytes[i * 4], read_file_bytes[(i * 4) + 1], read_file_bytes[(i * 4) + 2]]),
				read_file_bytes[(i * 4) + 3],
			);
			timestamps_table[i] = u32::from_be_bytes([
				read_file_bytes[(i * 4) + 1024],
				read_file_bytes[(i * 4) + 1025],
				read_file_bytes[(i * 4) + 1026],
				read_file_bytes[(i * 4) + 1027],
			]);
		}

		locations_table.iter().enumerate().filter(|(_i, (offset, length))| *offset != 0 && *length != 0).for_each(|(i, (offset, length))| {
	...
	for chunk in chunks {
		let mut chunk_nbt_tags = vec![
			...
			NbtTag::TagCompound(
				...
				vec![NbtTag::Int("max_section".to_string(), 20), NbtTag::Int("min_section".to_string(), -4)],
			),
			NbtTag::List(
				"sections".to_string(),
				chunk
					...
					.map(|(i, section)| {
						...
						let mut biome_data = vec![NbtTag::List(
							"palette".to_string(),
							biome_palette
								.iter()
								.map(|biome| {
						...
						if biome_palette.len() > 1 {
							biome_data.push(NbtTag::LongArray(
								"data".to_string(),
								section
									.biomes
									.iter()
									.map(|biome| biome_palette.iter().enumerate().find(|x| *x.1 == *biome).unwrap().0 as u8)
									.collect::<Vec<u8>>()
									.chunks(64 / biomes_bits_per_entry)
									.map(|byte_arr| {
										let mut long = 0u64;
						...
						let mut block_data = vec![NbtTag::List(
							"palette".to_string(),
							block_palette
								.iter()
								.map(|blockstate_id| {
									let block = block_states.iter().find(|x| x.1.states.iter().any(|x| x.id == *blockstate_id)).unwrap();
									let mut children = vec![NbtTag::String("Name".to_string(), block.0.clone())];

									if block.1.states.len() > 1 {
										children.push(NbtTag::TagCompound(
											"Properties".to_string(),
											data::blocks::get_raw_properties_from_block_state_id(block_states, *blockstate_id)
												.into_iter()
												.map(|x| NbtTag::String(x.0, x.1))
												.collect(),
										));
									}
						...
						if block_palette.len() > 1 {
							block_data.push(NbtTag::LongArray(
								"data".to_string(),
								section
									.blocks
									.iter()
									.map(|block| block_palette.iter().enumerate().find(|x| *x.1 == *block).unwrap().0 as u8)
									.collect::<Vec<u8>>()
									.chunks(64 / blocks_bits_per_entry)
									.map(|byte_arr| {
										let mut long = 0u64;
										for byte in byte_arr {
		...
		if !chunk.block_entities.is_empty() {
			let block_entities_nbt =
				NbtTag::List("block_entities".to_string(), chunk.block_entities.iter().map(|block_entity| block_entity.clone().into()).collect());
