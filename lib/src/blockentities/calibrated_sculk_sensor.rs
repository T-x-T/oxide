...
impl CommonBlockEntity for CalibratedSculkSensor {
...
impl From<CalibratedSculkSensor> for Vec<NbtTag> {
	fn from(_value: CalibratedSculkSensor) -> Self {
		return Vec::new();
	}
}

impl TryFrom<NbtListTag> for CalibratedSculkSensor {
	...
	fn try_from(value: NbtListTag) -> Result<Self, Self::Error> {
		...
		return Ok(CalibratedSculkSensor {
			position,
			components: Vec::new(),
		});
	}
