			...
			if self.lit_time_remaining == 0 {
				if self.inventory[1].count > 0 {
					...
					self.lit_time_remaining =
						game.recipe_manager.get_fuel_burning_time(data::items::get_item_name_by_id(self.inventory[1].id)) as i16;
				} else {
			...
			players.iter().filter(|x| x.opened_inventory_at.is_some_and(|y| y == self.position)).for_each(|x| {
				...
				game.send_packet(
					...
					crate::packets::clientbound::play::SetContainerProperty {
						...
						value: game.recipe_manager.get_fuel_burning_time(data::items::get_item_name_by_id(self.inventory[1].id)) as i16, //ticks fuel should burn for
					}
					.try_into()
					.unwrap(),
				);
