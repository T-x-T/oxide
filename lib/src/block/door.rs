...
pub fn get_block_state_id(
	...
	let block_id_of_valid_door = block
		...
		.iter()
		.find(|x| {
			x.properties.contains(&Property::DoorFacing(facing.clone()))
				&& x.properties.contains(&Property::DoorHalf(DoorHalf::Lower))
				&& x.properties.contains(&Property::DoorHinge(DoorHinge::Left))
				&& x.properties.contains(&Property::DoorOpen(open.clone()))
				&& x.properties.contains(&Property::DoorPowered(powered.clone()))
		})
		.unwrap()
		.id;
	let we_must_go_double_door_mode = block_id_next_to_door == block_id_of_valid_door;
	let hinge =
		if we_must_go_double_door_mode { if hinge_side == DoorHinge::Right { DoorHinge::Left } else { DoorHinge::Right } } else { hinge_side };
	...
	output.push((
		block
			...
			.find(|x| {
				x.properties.contains(&Property::DoorFacing(facing.clone()))
					&& x.properties.contains(&Property::DoorHalf(DoorHalf::Lower))
					&& x.properties.contains(&Property::DoorHinge(hinge.clone()))
					&& x.properties.contains(&Property::DoorOpen(open.clone()))
					&& x.properties.contains(&Property::DoorPowered(powered.clone()))
			})
			.unwrap()
	...
	output.push((
		block
			...
			.find(|x| {
				x.properties.contains(&Property::DoorFacing(facing.clone()))
					&& x.properties.contains(&Property::DoorHalf(DoorHalf::Upper))
					&& x.properties.contains(&Property::DoorHinge(hinge.clone()))
					&& x.properties.contains(&Property::DoorOpen(open.clone()))
					&& x.properties.contains(&Property::DoorPowered(powered.clone()))
			})
...
mod test {
	...
	mod interact {
		...
		fn open_door() {
			...
			let block_state_id_lower_closed = block
				.states
				.iter()
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::East))
						&& x.properties.contains(&Property::DoorHalf(DoorHalf::Lower))
						&& x.properties.contains(&Property::DoorHinge(DoorHinge::Left))
						&& x.properties.contains(&Property::DoorOpen(DoorOpen::False))
						&& x.properties.contains(&Property::DoorPowered(DoorPowered::False))
				})
				.unwrap()
				.id;

			let block_state_id_lower_opened = block
				.states
				.iter()
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::East))
						&& x.properties.contains(&Property::DoorHalf(DoorHalf::Lower))
						&& x.properties.contains(&Property::DoorHinge(DoorHinge::Left))
						&& x.properties.contains(&Property::DoorOpen(DoorOpen::True))
						&& x.properties.contains(&Property::DoorPowered(DoorPowered::False))
				})
				.unwrap()
				.id;
			let block_state_id_upper_opened = block
				.states
				.iter()
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::East))
		...
		fn close_door() {
			...
			let block_state_id_lower_closed = block
				...
				.unwrap()
				.id;
			let block_state_id_upper_closed = block
				.states
				.iter()
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::East))
	...
	mod get_block_state_id {
		...
		fn hinge_left_north() {
			...
			let block_state_id_lower = block
				...
				.iter()
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::North))
						&& x.properties.contains(&Property::DoorHalf(DoorHalf::Lower))
						&& x.properties.contains(&Property::DoorHinge(DoorHinge::Left))
						&& x.properties.contains(&Property::DoorOpen(DoorOpen::False))
						&& x.properties.contains(&Property::DoorPowered(DoorPowered::False))
				...
				.unwrap()
				.id;
			let block_state_id_upper = block
				.states
				.iter()
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::North))
		...
		fn hinge_left_west() {
			...
			let block_state_id_lower = block
				...
				.iter()
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::West))
						&& x.properties.contains(&Property::DoorHalf(DoorHalf::Lower))
						&& x.properties.contains(&Property::DoorHinge(DoorHinge::Left))
						&& x.properties.contains(&Property::DoorOpen(DoorOpen::False))
						&& x.properties.contains(&Property::DoorPowered(DoorPowered::False))
		...
		fn hinge_right_north() {
			...
			let block_state_id_lower = block
				...
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::North))
						&& x.properties.contains(&Property::DoorHalf(DoorHalf::Lower))
						&& x.properties.contains(&Property::DoorHinge(DoorHinge::Right))
						&& x.properties.contains(&Property::DoorOpen(DoorOpen::False))
						&& x.properties.contains(&Property::DoorPowered(DoorPowered::False))
				})
				.unwrap()
		...
		fn double_door() {
			...
			let block_state_id_upper = block
				...
				.unwrap()
				.id;
			let block_state_id_lower_double = block
				.states
				.iter()
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::North))
				...
				.unwrap()
				.id;
			let block_state_id_upper_double = block
				.states
				.iter()
				.find(|x| {
					x.properties.contains(&Property::DoorFacing(DoorFacing::North))
