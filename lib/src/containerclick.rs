...
pub fn handle(
	...
	let orig_cursor_item: Option<Slot> = players.iter().find(|x| x.uuid == player_uuid).unwrap().cursor_item.clone();
	...
	if let Some(orig_inventory_item) = orig_inventory_item.clone() {
		...
		if let Some(orig_cursor_item) = orig_cursor_item.clone() {
			...
			if (left_click) || orig_inventory_item.item_count < 2 {
				...
				new_cursor_item =
					Some(Slot { item_count: (f64::from(orig_inventory_item.item_count) / 2.0).ceil() as i32, ..orig_inventory_item.clone() });
			}
		}
	} else {
...
mod test {
	...
	mod handle_click {
		...
		fn stack_up_under_limit() {
			...
			assert_eq!(
				new_inventory_item,
				Some(Slot { item_count: 13, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
			);
		...
		fn stack_up_to_limit() {
			...
			assert_eq!(
				new_inventory_item,
				Some(Slot { item_count: 64, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
			);
			assert_eq!(new_cursor_item, None);
		}
		...
		fn stack_up_over_limit() {
			...
			assert_eq!(
				new_cursor_item,
				Some(Slot { item_count: 10, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
			);
		}
		...
		fn pick_half_stack_up_even() {
			...
			assert_eq!(
				new_inventory_item,
				Some(Slot { item_count: 5, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
			);
			assert_eq!(
				new_cursor_item,
				Some(Slot { item_count: 5, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
		...
		fn pick_half_stack_up_odd() {
			...
			assert_eq!(
				new_cursor_item,
				Some(Slot { item_count: 6, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
			);
		}
		...
		fn pick_half_stack_up_one() {
			...
			assert_eq!(
				new_cursor_item,
				Some(Slot { item_count: 1, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
			);
		}
		...
		fn put_one_item_down() {
			...
			assert_eq!(
				new_cursor_item,
				Some(Slot { item_count: 9, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
			);
		}
		...
		fn put_one_item_down_with_same_already_there() {
			...
			assert_eq!(
				new_inventory_item,
				Some(Slot { item_count: 11, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
			);
			assert_eq!(
				new_cursor_item,
				Some(Slot { item_count: 9, item_id: 1, components_to_add: Vec::new(), components_to_remove: Vec::new() })
	...
	mod handle_shift_click {
		...
		fn shift_click_to_chest_full() {
			let orig_chest_inventory: Vec<Option<Slot>> =
				vec![vec![Some(Slot { item_id: 1, item_count: 12, components_to_add: Vec::new(), components_to_remove: Vec::new() }); 27]]
					.into_iter()
					.flatten()
					.collect();
			let orig_player_inventory: Vec<Option<Slot>> = vec![
		...
		fn shift_click_to_inventory_full() {
			...
			let orig_player_inventory: Vec<Option<Slot>> =
				vec![vec![Some(Slot { item_id: 2, item_count: 12, components_to_add: Vec::new(), components_to_remove: Vec::new() }); 46]]
					.into_iter()
					.flatten()
					.collect::<Vec<Option<Slot>>>();
			let clicked_slot = 0;
