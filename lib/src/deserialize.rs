	...
	for _ in 0..number_of_components_to_add {
		...
		components_to_add.push(match component_id {
			...
			14 => SlotComponent::CustomModelData(
				(0..varint(data)?).map(|_| float(data).unwrap()).collect(),
				(0..varint(data)?).map(|_| boolean(data).unwrap()).collect(),
				(0..varint(data)?).map(|_| string(data).unwrap()).collect(),
				(0..varint(data)?).map(|_| int(data).unwrap()).collect(),
			),
			...
			45 => SlotComponent::WritableBookContent(
				(0..varint(data)?)
					.map(|_| (string(data).unwrap(), if boolean(data).unwrap() { Some(string(data).unwrap()) } else { None }))
					.collect(),
			),
			...
			61 => SlotComponent::Profile(
				if boolean(data)? { Some(string(data)?) } else { None },
				if boolean(data)? { Some(uuid(data)?) } else { None },
				(0..varint(data)?)
					.map(|_| (string(data).unwrap(), string(data).unwrap(), if boolean(data).unwrap() { Some(string(data).unwrap()) } else { None }))
					.collect(),
			),
			...
			68 => SlotComponent::Bees(
				(0..varint(data)?).map(|_| (nbt_network(data).unwrap(), varint(data).unwrap(), varint(data).unwrap())).collect(),
			),
...
mod test {
	...
	fn nbt_mobspawner() {
		let nbt_parsed = NbtTag::Root(vec![
			...
			NbtTag::TagCompound(
				"SpawnData".to_string(),
				vec![NbtTag::TagCompound("entity".to_string(), vec![NbtTag::String("id".to_string(), "minecraft:spider".to_string())])],
			),
		...
		let mut nbt_bytes: Vec<u8> = vec![
			10, 2, 0, 17, 77, 97, 120, 78, 101, 97, 114, 98, 121, 69, 110, 116, 105, 116, 105, 101, 115, 0, 6, 2, 0, 19, 82, 101, 113, 117, 105,
			114, 101, 100, 80, 108, 97, 121, 101, 114, 82, 97, 110, 103, 101, 0, 16, 2, 0, 10, 83, 112, 97, 119, 110, 67, 111, 117, 110, 116, 0,
			4, 10, 0, 9, 83, 112, 97, 119, 110, 68, 97, 116, 97, 10, 0, 6, 101, 110, 116, 105, 116, 121, 8, 0, 2, 105, 100, 0, 16, 109, 105, 110,
			101, 99, 114, 97, 102, 116, 58, 115, 112, 105, 100, 101, 114, 0, 0, 2, 0, 13, 77, 97, 120, 83, 112, 97, 119, 110, 68, 101, 108, 97,
			121, 3, 32, 2, 0, 10, 83, 112, 97, 119, 110, 82, 97, 110, 103, 101, 0, 4, 2, 0, 5, 68, 101, 108, 97, 121, 0, 20, 2, 0, 13, 77, 105,
			110, 83, 112, 97, 119, 110, 68, 101, 108, 97, 121, 0, 200, 0,
		];
	...
	fn nbt_nested_compound_network() {
		let nbt = NbtTag::Root(vec![NbtTag::TagCompound(
			"a".to_string(),
			vec![NbtTag::TagCompound("b".to_string(), vec![NbtTag::String("c".to_string(), "hi!".to_string())])],
		)]);
	...
	fn nbt_basic_network() {
		let nbt = NbtTag::Root(vec![NbtTag::String("hi".to_string(), "hello".to_string()), NbtTag::Int("hi2".to_string(), 1234)]);
	...
	fn nbt_with_list_of_compounds_network() {
		let nbt = NbtTag::Root(vec![NbtTag::List(
			"this_is_a_list".to_string(),
			vec![
				NbtListTag::TagCompound(vec![NbtTag::String("a".to_string(), "b".to_string())]),
				NbtListTag::TagCompound(vec![NbtTag::String("a".to_string(), "b".to_string())]),
				NbtListTag::TagCompound(vec![NbtTag::String("a".to_string(), "b".to_string())]),
			],
		)]);
	...
	fn nbt_with_empty_list_disk() {
		let nbt = NbtTag::Root(vec![NbtTag::List("this_is_a_list".to_string(), vec![])]);
	...
	fn nbt_empty_tag_compound() {
		let nbt = NbtTag::Root(vec![NbtTag::TagCompound("abc".to_string(), vec![])]);
	...
	fn nbt_empty_tag_compound_in_list_in_compound() {
		let nbt = NbtTag::Root(vec![NbtTag::List(
			"abc".to_string(),
			vec![NbtListTag::TagCompound(vec![NbtTag::TagCompound("def".to_string(), vec![])])],
		)]);
