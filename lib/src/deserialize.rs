...
fn nbt_tag_compound(mut data: &mut Vec<u8>, has_description: bool, has_id: bool) -> Result<NbtTag, Box<dyn Error>> {
  ...
  loop {
    ...
    match id {
      ...
      0x01 => {
        println!("decode byte");
        tags.push(NbtTag::Byte(None, data.pop().unwrap()));
      },
      0x02 => {
        println!("decode short");
        data.reverse();
        tags.push(NbtTag::Short(None, short(&mut data)?));
        data.reverse();
      },
      0x03 => {
        println!("decode int");
        data.reverse();
        tags.push(NbtTag::Int(None, int(&mut data)?));
        data.reverse();
      },
      0x04 => {
        println!("decode long");
        data.reverse();
        tags.push(NbtTag::Long(None, long(&mut data)?));
        data.reverse();
      },
      0x05 => {
        println!("decode float");
        data.reverse();
        tags.push(NbtTag::Float(None, float(&mut data)?));
        data.reverse();
      },
      0x06 => {
        println!("decode double");
        data.reverse();
        tags.push(NbtTag::Double(None, double(&mut data)?));
        data.reverse();
      },
      0x07 => {
        println!("decode byte array");
        data.reverse();
        tags.push(NbtTag::ByteArray(None, nbt_byte_array_value(&mut data)?));
        data.reverse();
      },
      0x08 => {
        println!("decode string");
        data.reverse();
        tags.push(NbtTag::String(None, nbt_string_value(&mut data)?));
        data.reverse();
      },
      0x09 => {
        println!("decode list");
      ...
      0x0b => {
        ...
        data.reverse();
        tags.push(NbtTag::IntArray(None, nbt_int_array_value(&mut data)?));
        data.reverse();
      },
      0x0c => {
