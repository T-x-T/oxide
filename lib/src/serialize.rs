	...
	for component_to_add in &input.components_to_add {
		...
		output.append(&mut match component_to_add.clone() {
			...
			SlotComponent::CustomModelData(a, b, c, d) => vec![
				a.into_iter().flat_map(float).collect::<Vec<u8>>(),
				b.into_iter().flat_map(boolean).collect(),
				c.into_iter().flat_map(|x| string(&x)).collect(),
				d.into_iter().flat_map(int).collect(),
			]
			.into_iter()
			.flatten()
			.collect(),
			...
			SlotComponent::UseCooldown(a, b) => {
				vec![float(a), if let Some(b) = b { vec![vec![1], string(&b)].into_iter().flatten().collect() } else { vec![0] }]
					.into_iter()
					.flatten()
					.collect()
			}
			...
			SlotComponent::WritableBookContent(a) => a
				.into_iter()
				.flat_map(|(x, y)| {
					vec![string(&x), if let Some(y) = y { vec![vec![0x01], string(&y)].into_iter().flatten().collect() } else { vec![0x00] }]
				})
				.flatten()
				.collect(),
			...
			SlotComponent::Profile(a, b, c) => vec![
				if let Some(a) = a { vec![vec![0x01], string(&a)].into_iter().flatten().collect() } else { vec![0x00] },
				...
				c.into_iter()
					.flat_map(|(x, y, z)| {
						vec![
							string(&x),
							string(&y),
							if let Some(z) = z { vec![vec![0x01], string(&z)].into_iter().flatten().collect() } else { vec![0x00] },
						]
					})
					.flatten()
					.collect(),
