...
pub fn slot(input: Option<&Slot>) -> Vec<u8> {
  ...
  for component_to_add in &input.components_to_add {
    ...
    output.append(&mut match component_to_add.clone() {
      ...
      SlotComponent::UseCooldown(a, b) => vec![float(a), if let Some(b) = b {vec![vec![1], string(&b)].into_iter().flatten().collect()} else {vec![0]}].into_iter().flatten().collect(),
      ...
      SlotComponent::WritableBookContent(a) => a.into_iter().flat_map(|(x, y)| vec![string(&x), if let Some(y) = y {vec![vec![0x01], string(&y)].into_iter().flatten().collect()} else {vec![0x00]}]).flatten().collect(),
      SlotComponent::WrittenBookContent(a) => a.into_iter().flat_map(|(x, y)| vec![string(&x), if let Some(y) = y {vec![vec![0x01], string(&y)].into_iter().flatten().collect()} else {vec![0x00]}]).flatten().collect(),
      ...
      SlotComponent::Profile(a, b, c) => vec![if let Some (a) = a {vec![vec![0x01], string(&a)].into_iter().flatten().collect()} else {vec![0x00]}, if let Some(b) = b {vec![vec![0x01], uuid(&b)].into_iter().flatten().collect()} else {vec![0x00]}, c.into_iter().flat_map(|(x, y, z)| vec![string(&x), string(&y), if let Some(z) = z {vec![vec![0x01], string(&z)].into_iter().flatten().collect()} else {vec![0x00]}]).flatten().collect()].into_iter().flatten().collect(),
