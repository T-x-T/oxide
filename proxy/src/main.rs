				...
				match current_state {
					...
					lib::ConnectionState::Configuration => {
						if packet_id == lib::packets::serverbound::configuration::ServerboundKnownPackets::PACKET_ID {
							let parsed_packet =
								lib::packets::serverbound::configuration::ServerboundKnownPackets::try_from(server_packet.data.clone()).unwrap();
							println!("parsed packet: {parsed_packet:?}");
							parsed_server_packet = Some(parsed_packet.try_into().unwrap());
						}
						if packet_id == lib::packets::serverbound::configuration::AcknowledgeFinishConfiguration::PACKET_ID {
							let parsed_packet =
								lib::packets::serverbound::configuration::AcknowledgeFinishConfiguration::try_from(server_packet.data.clone()).unwrap();
							println!("parsed packet: {parsed_packet:?}");
					...
					lib::ConnectionState::Play => {
						match packet_id {
							...
							lib::packets::serverbound::play::SetPlayerPositionAndRotation::PACKET_ID => {
								let parsed_packet =
									lib::packets::serverbound::play::SetPlayerPositionAndRotation::try_from(server_packet.data.clone()).unwrap();
								// println!("parsed packet: {parsed_packet:?}");
								parsed_server_packet = Some(parsed_packet.try_into().unwrap());
							}
					...
					lib::ConnectionState::Configuration => {
						match packet_id {
							lib::packets::clientbound::configuration::ClientboundKnownPacks::PACKET_ID => {
								let parsed_packet =
									lib::packets::clientbound::configuration::ClientboundKnownPacks::try_from(client_packet.data.clone()).unwrap();
								println!("parsed packet: {parsed_packet:?}");
								parsed_client_packet = Some(parsed_packet.try_into().unwrap());
							}
							...
							lib::packets::clientbound::configuration::FinishConfiguration::PACKET_ID => {
								let parsed_packet =
									lib::packets::clientbound::configuration::FinishConfiguration::try_from(client_packet.data.clone()).unwrap();
								println!("parsed packet: {parsed_packet:?}");
								parsed_client_packet = Some(parsed_packet.try_into().unwrap());
							}
					...
					lib::ConnectionState::Play => {
						match packet_id {
							...
							lib::packets::clientbound::play::UpdateEntityPositionAndRotation::PACKET_ID => {
								let parsed_packet =
									lib::packets::clientbound::play::UpdateEntityPositionAndRotation::try_from(client_packet.data.clone()).unwrap();
								//println!("parsed packet: {parsed_packet:?}");
								parsed_client_packet = Some(parsed_packet.try_into().unwrap());
							}
							...
							lib::packets::clientbound::play::DebugEntityValue::PACKET_ID => {
								let parsed_packet = lib::packets::clientbound::play::DebugEntityValue::try_from(client_packet.data.clone()).unwrap();
								println!("parsed packet: {parsed_packet:?}");
								parsed_client_packet = Some(parsed_packet.try_into().unwrap());
							}
