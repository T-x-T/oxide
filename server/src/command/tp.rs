...
fn execute(command: String, stream: Option<&mut TcpStream>, game: Arc<Game>) -> Result<(), Box<dyn Error>> {
	...
	sending_player.new_position(target_coordinates.x, target_coordinates.y, target_coordinates.z, &mut game.world.lock().unwrap(), &game.entity_id_manager, &game.block_state_data, game.clone())?;

	sending_player.current_teleport_id += 1;
	game.send_packet(&stream.peer_addr()?, lib::packets::clientbound::play::SynchronizePlayerPosition::PACKET_ID, lib::packets::clientbound::play::SynchronizePlayerPosition {
  ...
  for other_stream in players.iter().map(|x| &x.connection_stream).collect::<Vec<&TcpStream>>() {
    if other_stream.peer_addr().unwrap() != stream.peer_addr().unwrap() {
      	game.send_packet(&other_stream.peer_addr()?, lib::packets::clientbound::play::TeleportEntity::PACKET_ID, lib::packets::clientbound::play::TeleportEntity {
